...
@Table(
    ...
    uniqueConstraints = [
        UniqueConstraint(name = "uk_borrowing_outbox_event_id", columnNames = ["event_id"]),
        UniqueConstraint(
            name = "uk_borrowing_outbox_transition",
            columnNames = ["aggregate_id", "event_type"]
        )
    ],
    indexes = [Index(name = "idx_borrowing_outbox_pending", columnList = "published_at, created_at")]
)
