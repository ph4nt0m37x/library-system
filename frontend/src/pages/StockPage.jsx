        ...
        try {
            ...
            if (accepted) {
                for (const [path, body] of [
                    ["ship", { id: transfer.id }],
                    ["complete", { id: transfer.id, completedAt: new Date().toISOString() }],
                ]) {
                    const response = await fetch(`${API}/api/transfers/${path}`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            Authorization: `Bearer ${token}`,
                    ...
                    if (!response.ok) {
                        const message = await response.text();
                        throw new Error(message || `Failed to ${path} transfer.`);
                    }
                }
            }

            if (accepted) {
                const expectedAvailable = availableBefore + transfer.quantity;
                ...
                {loadingTransfers ? (
                    ...
                    pendingTransfers.map((transfer) => (
                        <div key={transfer.id} className={styles.transferRequest}>
                            ...
                            <p>Requested by library: {transfer.destinationLibraryId}</p>
                            <div className={styles.rowActions}>
                                <button
                                    className={styles.primaryButton}
